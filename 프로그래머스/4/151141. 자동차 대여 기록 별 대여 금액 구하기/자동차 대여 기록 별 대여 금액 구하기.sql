...
(
    ...
    LEFT JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN T 
        ON S.CAR_TYPE = T.CAR_TYPE
        AND (
            (DATEDIFF(END_DATE, START_DATE) + 1 >= 90 AND DURATION_TYPE = '90일 이상') OR
            (DATEDIFF(END_DATE, START_DATE) + 1 >= 30 AND DATEDIFF(END_DATE, START_DATE) + 1 < 90 AND DURATION_TYPE = '30일 이상') OR
    ...
    UNION ALL
    # 할인 안되는거    
    SELECT HISTORY_ID, FLOOR(DAILY_FEE * (DATEDIFF(END_DATE, START_DATE) + 1)) AS F
    FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY CR
    LEFT JOIN CAR_RENTAL_COMPANY_CAR S ON CR.CAR_ID = S.CAR_ID 
